...
def GreedyMaxEntTag(input_file_name, model_name, feature_map_file, out_file_name):
    ...
    for line in feature_mapping_lines:
        if line != '' or line != '\n':
            ...
            if len(splitted_line) != 2:
                ...
                continue
            features_dict[splitted_line[0]] = int(splitted_line[1])
            features_dict_number_is_ind[int(splitted_line[1])] = splitted_line[0]

    #get list of tags
    list_of_tags = ['NNS', 'NN', 'NNP', 'NNPS', 'PRP', 'WP', 'VB', 'VBD', 'VBG', 'VBN', 'VBZ', 'VBP', 'MD', 'TO',
    ...
    for i in range(0, len(lines)):
        ...
        for j in range(0, len(words)):
            ...
            converted_feature = ConvertFeatures.ConvertFeaturesFromSentence(event, features_dict)
            result = loaded_model.predict([converted_feature])
            #print str(result)
            tags.insert(j, str(features_dict_number_is_ind[int(result[0])]))
